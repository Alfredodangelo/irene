// =============================================
//  PRESEDUTA BOOKING PAGE - Irene Gipsy Tattoo
//  Integrazione: Cal.com API + n8n
// =============================================
...
// Prima seduta e deadline
let firstSessionDateStr = null; // 'YYYY-MM-DD'
let maxBookingDateStr = null;   // 'YYYY-MM-DD' (prima seduta - 20 giorni)
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Leggi URL param firstSessionDate
    ...
    if (fsParam) {
        ...
        // Auto-detect prima seduta + profilo da Supabase se loggato
        ...
        try {
            ...
            if (userId) {
                ...
                // Pre-compila e nascondi i campi personali
                ...
                if (firstName || email) {
                    ...
                    document.getElementById('bookingAsText').textContent = `Stai prenotando come: ${displayName} (${email})`;
                    document.getElementById('bookingAsInfo').style.display = 'flex';
                }
            }
        } catch (e) {
            console.warn('[preseduta] auto-detect:', e);
        }
        if (!autoDetected) {
...
function renderFirstSessionBanner(dateStr) {
    ...
    const maxLabel = maxObj.toLocaleDateString('it-IT', { day: 'numeric', month: 'long', year: 'numeric' });

    section.innerHTML = `
        <div class="first-session-banner">
            <i class="fas fa-calendar-check"></i>
            <div>
                <strong>Prima seduta: ${dateLabel}</strong>
                <p>La pre-seduta deve essere prenotata almeno ${CONFIG.MIN_DAYS_BEFORE_SESSION} giorni prima — entro il <strong>${maxLabel}</strong>.</p>
            </div>
        </div>
    `;
}
...
function renderFirstSessionInput() {
    const section = document.getElementById('firstSessionSection');
    // La data minima della prima seduta = oggi + 20 giorni + 1 (almeno 1 giorno disponibile)
    ...
    section.innerHTML = `
        <h3 class="booking-card-title">
            <i class="far fa-calendar-alt"></i> Data Prima Seduta
        </h3>
        <p style="font-size:0.9rem; color:#aaa; margin-bottom:14px;">
            Inserisci la data della tua prima seduta per visualizzare le date disponibili per la pre-seduta (almeno ${CONFIG.MIN_DAYS_BEFORE_SESSION} giorni prima).
        </p>
        <div class="form-group">
            <label for="firstSessionInput">Data della prima seduta *</label>
    ...
    document.getElementById('firstSessionInput').addEventListener('change', function () {
        if (this.value) {
            ...
            renderCalendar();
            // Ricarica mese corrente con i nuovi filtri
...
function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const isLocked = maxBookingDateStr === null; // nessuna data prima seduta impostata
...
function selectTime(isoTime, btn) {
    ...
    document.getElementById('selectedInfoText').textContent = `${dateLabel} ore ${timeLabel}`;
...
async function handleSubmit(e) {
    ...
    if (!isFormValid()) {
        const formMsg = document.getElementById('formMsg');
        formMsg.textContent = 'Compila tutti i campi obbligatori prima di procedere.';
        formMsg.className = 'form-message error';
        return;
    }

    // Verifica: solo una pre-seduta attiva per cliente
    if (window.__authUser) {
        ...
        if (existingPreseduta) {
            const formMsg = document.getElementById('formMsg');
            formMsg.textContent = 'Hai già una pre-seduta prenotata. Puoi averne solo una attiva alla volta.';
            formMsg.className = 'form-message error';
    ...
    submitBtn.innerHTML = '<span class="loading-spinner"></span> Prenotazione in corso...';
    ...
    if (luogo === 'integrations:whatsapp_video' && !motivoVideo) {
        ...
        submitBtn.innerHTML = '<i class="fas fa-check"></i> Conferma Pre-Seduta';
        return;
    }
    ...
    try {
        ...
        const formMsg = document.getElementById('formMsg');
        formMsg.textContent = 'Si è verificato un errore. Riprova o contatta Irene direttamente.';
        formMsg.className = 'form-message error';
        state.isSubmitting = false;
        submitBtn.disabled = false;
        submitBtn.innerHTML = '<i class="fas fa-check"></i> Conferma Pre-Seduta';
...
async function createCalBooking(formData) {
    ...
    const notes = [
        formData.note || '',
        firstSessionDateStr ? `Prima seduta: ${firstSessionDateStr}` : '',
        formData.motivoVideo ? `Motivo video: ${formData.motivoVideo}` : '',
    ].filter(Boolean).join('\n');
...
async function sendToN8n(formData, bookingResult) {
    ...
    const params = new URLSearchParams({
        ...
        note: formData.note || '',
        data_preseduta: `${dateLabel} ore ${timeLabel}`,
...
async function savePresedutaToSupabase(bookingResult, formData) {
    ...
    try {
        ...
        if (error) console.warn('[preseduta.js] Supabase insert error:', error.message);
    } catch (err) {
        console.warn('[preseduta.js] Supabase save failed:', err);
    }
}
...
function showSuccessState(bookingResult, userData) {
    ...
    let detailsHTML = `
        ...
        <div class="booking-detail-item"><i class="far fa-calendar-alt"></i><span>${dateLabel} ore ${timeLabel}</span></div>
