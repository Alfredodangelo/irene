#!/usr/bin/env node
// ============================================
//  Push Notification Micro-Server
//  Gira sul VPS accanto a n8n, porta 3099
//  n8n lo chiama via HTTP Request localhost
// ============================================
...
async function handleSend(body) {
  ...
  if (!user_id) return { error: 'user_id mancante' };
  try {
    const subs = await getSubs(user_id);
    if (!Array.isArray(subs) || !subs.length) return { sent: 0, total: 0 };
    const payload = JSON.stringify({
      title: title || 'Irene Gipsy Tattoo',
      body: message || '',
    ...
    for (const s of subs) {
      ...
      try {
        ...
        res.push({ st: r.s >= 200 && r.s < 300 ? 'sent' : 'err', c: r.s });
      } catch (e) { res.push({ st: 'err', e: e.message }); }
    }
    return { sent: res.filter(r => r.st === 'sent').length, total: subs.length, results: res };
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/health') {
    res.end(JSON.stringify({ status: 'ok', service: 'igt-push' }));
    return;
  }
  ...
  if (req.method === 'POST' && req.url === '/send') {
    ...
    req.on('end', async () => {
      try {
        ...
        res.statusCode = 400;
        res.end(JSON.stringify({ error: 'JSON non valido: ' + e.message }));
      }
    });
    return;
  }
...
server.listen(PORT, '127.0.0.1', () => {
  console.log('[IGT Push Server] In ascolto su http://127.0.0.1:' + PORT);
});
